...
export default function IntegrationsAdminPage() {
  ...
  const [secretForm, setSecretForm] =
    useState<SecretFormState>(emptySecretForm);
  ...
  const handleSaveSecrets = async (event: FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/api/integration-accounts/${selectedAccountId}/secret`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      <header className="border-b bg-white px-6 py-6 shadow-sm">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-2">
          ...
          <p className="text-sm text-gray-500">
            Adicione WhatsApp via QR ou API Evolution.
          </p>
          <Link
            href="/dashboard"
          ...
          <section className="rounded-xl border bg-white p-6 shadow-sm">
            ...
            <form onSubmit={handleSaveSecrets} className="mt-4 space-y-3">
              ...
              <select
                ...
                onChange={handleSecretChange("provider")}
                disabled={!activeAccount}
              >
              ...
              <input
                ...
                value={secretForm.sessionName}
                onChange={handleSecretChange("sessionName")}
                disabled={!activeAccount}
              />
