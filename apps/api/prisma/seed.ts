...
async function seedAutomationTemplates(adminId: string) {
  ...
  if (!hasAllMarketplaceTables) {
    console.log(
      'Seed marketplace ignorado: tabelas ausentes.',
      marketplaceTables.filter((table) => !table.exists).map((table) => table.tableName).join(', '),
    );
    return;
  }
  ...
  try {
    ...
    if (isMissingTableError(error)) {
      console.log('Seed marketplace ignorado: tabela MarketplaceCategory ausente.');
      return;
    }
