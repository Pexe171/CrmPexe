...
@Injectable()
export class FacebookMessengerProvider implements IChannelProvider {
  ...
  async sendMessage(
    ...
    if (!apiUrl || !apiToken) {
      throw new BadRequestException(
        "Integração do Facebook Messenger sem apiUrl ou apiToken. Configure a Evolution API para Messenger."
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new BadRequestException(`Falha ao enviar Messenger: ${errorBody}`);
      }
    ...
    return {
      providerMessageId: responseBody?.id ?? `fb_${Date.now()}`,
      raw: responseBody
    };
  }
  ...
  async receiveWebhook(
    ...
    return data.messages
      ...
      .map((message) => ({
        id: message.id ?? `fb_${Date.now()}_${message.from}`,
        text: message.text ?? "",
  ...
  mapInboundToContact(message: ChannelInboundMessage): ChannelContact {
    return {
      name: message.contact.name ?? "Contato Messenger",
